...
bool fs_load_gw()
{
    ...
    // open config file for reading
    File configFile = LittleFS.open(gw_cfg_file, "r");
    if (!configFile)
        return false;
    ...
    if (doc.containsKey("enb_max_time"))
        enb_max_time = doc["enb_max_time"];
...
const char *fs_save_gw()
{
    ...
    doc["enb_max_time"] = enb_max_time;

    yield();

    // serialize & write config
    File configFile = LittleFS.open(gw_cfg_file, "w");
    if (!configFile)
        return file_write_failure;
